...
export default function OptionsList({
  ...
  const [displayedHighlightedIndex, setHighlightedIndex] = useState<number>(-1);

  const highlightedIndex = useMemo(() => {
    if (filteredOptions.length === 0) return -1;
    return Math.min(displayedHighlightedIndex, filteredOptions.length - 1);
  }, [filteredOptions.length, displayedHighlightedIndex]);
