...
export default function useDates(options: UseDatesOptions): UseDatesResult {
  ...
  const [selectedPeriod, setSelectedPeriod] = useState<PeriodId>(defaultPeriod);
  ...
  const [start, setStart] = useState<Dayjs>(
    () => computeRangeForPeriod(defaultPeriod, today, oldestInceptionDay).start,
  );
  ...
  return {
    oldestInceptionDate: oldestInceptionDay ? iso(oldestInceptionDay) : null,
    availablePeriods,
    selectedPeriod,
    startDate: iso(start),
    endDate: iso(end),
