...
export default function useTanstackQuery({
  ...
  const invalidate = useCallback(
    async (
      ...
      await queryClient.invalidateQueries({
        ...
        exact: exactMatch,
        refetchType:
          options.refetchActive === false && options.refetchInactive === false
            ? "none"
  ...
  const setData = useCallback(
    <TData = unknown>(updater: TData | ((old: TData | undefined) => TData)) => {
  ...
  const appendData = useCallback(
    <TData = unknown>(newItem: TData extends Array<infer U> ? U : never) => {
  ...
  const removeDataItem = useCallback(
    <TData = unknown>(
      ...
      queryClient.setQueryData<TData>(queryKey, (old) => {
        if (Array.isArray(old)) {
          return old.filter((item, idx) => !predicate(item, idx)) as TData;
        }
        return old;
      });
    },
