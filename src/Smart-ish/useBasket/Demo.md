...
export default function SwapiDemo() {
  ...
  return (
    <>
      <Font.Bold size="lg">
        Load Options{" "}
        <Font.Bold size="lg" component="span" dangerousColor="var(--positive)">
          {RESOURCES.length}
        </Font.Bold>
      </Font.Bold>
      <br />
      <Toolbar label="pick from resources" dangerous={{ gap: "var(--gap-1)" }}>
      ...
      >
        ...
        {items.map((item, index) => {
          ...
          return (
            <Group.Row
              key={item.id}
              className={clsx(isInBasket && "positive", "theme-transparent")}
              dangerous={{
            ...
            >
              ...
              >
                {isInBasket ? <IconX size={22} /> : <IconAdd size={22} />}
              </Button>
            </Group.Row>
          );
        })}
